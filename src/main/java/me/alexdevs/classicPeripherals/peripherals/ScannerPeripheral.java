...
public class ScannerPeripheral implements IPeripheral {
    ...
    public final String getLabel() {
        ...
        var title = PrintoutItem.getTitle(stack);
    ...
    public final int getPageCount() {
        ...
        return PrintoutItem.getPageCount(stack);
    }
    ...
    public final ObjectLuaTable scan() {
        ...
        var pageCount = PrintoutItem.getPageCount(printout);
        var textLines = PrintoutItem.getText(printout);
        var colorLines = PrintoutItem.getColours(printout);
        ...
        //      ]
        // ]
        var table = extractPages(pageCount, textLines, colorLines);
        return table;
    }

    private static @NonNull ObjectLuaTable extractPages(int pageCount, String[] textLines, String[] colorLines) {
        ...
        for (var i = 0; i < pageCount; i++) {
            ...
            for (var j = i * PrintoutItem.LINES_PER_PAGE; j < (i + 1) * PrintoutItem.LINES_PER_PAGE; j++) {
                texts.put(j + 1, textLines[j]);
                colors.put(j + 1, colorLines[j]);
            }
        }
