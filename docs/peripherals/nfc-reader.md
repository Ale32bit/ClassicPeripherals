...
A NFC Card is capable of holding string data up to **128 characters** (configurable) and the `readonly` flag can be irreversibly enabled to prevent further writes.
...
Using a NFC Card with data on a player will enqueue the [`nfc_data`](#nfc_data) event to all pocket computers of the target player, with the side parameter being `"internal"`.
...
Interacting with a NFC Reader in `write` mode (yellow status light) will emit the [`nfc_data`](#nfc_data) event on the pocket computer.
