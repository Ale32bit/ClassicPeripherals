...
- Range decreased to 2x (configurable) of normal wireless modems, unaffected by weather.
- Cross-dimensional messaging is limited to *local* block coordinates. That is:
    - If an Ender Modem is placed in the overworld at the coordinates **150 64 150**, the messages are transmitted to
      the other dimensions from the same block coordinates, with a range of eight blocks. Coordinate scaling still applies.
...
Whenever a Redstone Relay peripheral receives a redstone input change, the `redstone` event is fired alone without any
parameter.
