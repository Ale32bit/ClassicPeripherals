package me.alexdevs.classicPeripherals.core.dataHolder;
...
public class DataHolderHandler {
    ...
    public static Optional<String> getPrivateKey(ItemStack stack) {
        if (stack.getItem() instanceof IDataHolder dataItem) {
    ...
    static void tryMigrate(ItemStack stack) {
        ...
        DataHolderHandler.setReadOnly(stack, legacyData.get().readonly());
        DataHolderHandler.setColor(stack, legacyData.get().color());
    ...
    record DataItemData(UUID uuid, String data, boolean readonly, int color, @Nullable String privateKey) {
        public static Optional<DataItemData> migrate(ItemStack stack) {
            if (!(stack.getItem() instanceof IDataHolder)) {
                return Optional.empty();
            }
